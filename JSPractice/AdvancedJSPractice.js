var cardData = [{
  ...
  function detectNetwork(cardNumber, cardData) {
	  // create credit card variable aliases
	  let cardLen = cardNumber.length;
	  let cardPrefix = cardNumber.slice(0,1);
	  // iterate over the cardData array
	  for (var i = 0; i < cardData.length; i++) {
		  //create variable alias for provider lengths array
		  let providerLengthArray = cardData[i].lengths;
		  //create variable alias for provider prefix array
		  let providerPrefix = cardData[i].prefixes;
		  //iterate through the provider length & prefix arrays, to return provider match
		  for (var j = 0; j < providerLengthArray.length; j++) {
			  //if card length is 16, card can only be Visa or Mastercard
			  if (cardLen === 16) {
				  //iterate through prefixes array to return Visa if prefix is 4
				  for (var k = 0; k < providerPrefix.length; k++) {
					  if (cardPrefix === providerPrefix[k]) {
				  ...
				  if (cardLen === providerLengthArray[j]) {
					  return cardData[i].network;
				  };
			  };
		  };
  ...
  var actualResults = [
	  detectNetwork('343456789012345', cardData),
	  detectNetwork('4357881561287565', cardData),
	  detectNetwork('5188576115752845', cardData),
	  detectNetwork('38885761157528', cardData)
  ];
  ...
  let testName = 'it renders the correct network provider for the provided input card data';
  
  
  console.log(assertArraysEqual(actualResults, expectedResults, testName));
  ...
  // 		//create shellObj to store each integer
  // 		let shellObj = {};
  // 		let integer = integers[i];
  // 		//add each integer as a key to shellObj
  // 		shellObj.integer = integers[i];
  // 		shellObj.pairsToEval = integers;
  // 		shellObj.resultSums = [];
  // 		//add shellObj to resultArray
  // 		evalArr.push(shellObj);
  // 	};
  // 	//create resultArr;
  // 	let resultArr = [];
  // 	//reevaluate over the evaluation array, to find the target sum
  // 	for (var j = 0; j < evalArr.length; j++) {
  // 		for (var k = 0; k < evalArr[j].pairsToEval.length; k++) {
  // 			if (evalArr[j].integer !== evalArr[j].pairsToEval[k]) {
  // 				let result = evalArr[j].integer + evalArr[j].pairsToEval[k];
  // 				if (result === target) {
  // 					evalArr[j].resultSums.push(result);
  // 					resultArr.push(evalArr[j].integer, evalArr[j].pairsToEval[k]);
  // 					break;
  // 				};
  // 			};
  // 		};
  // 	};
  // 	//return resultArr, if there are results
  // 	if (resultArr.length >= 2) {
  // 		return resultArr.slice(0,2);
  // 	} else {
  // 		return "No unique integer pairs equal the target sum";
  // 	};
  // };
  ...
  // //Test Suite
  // function assertArraysEqual(actual, expected, testName) {
  // 	let success = `passed [${testName}] : expected "${expected}" and got "${actual}" \n NICE JOB BRO!!! \n HOURS OF WORK AND YOU CRACKED THE CODE!`;
  // 	let failure = `failed [${testName}] : expected "${expected}", but got "${actual}"`;
  // 	let hasEqualLengths = true;
  ...
  // 		};
  // 	};
  // 	if (hasEqualLengths && hasEqualValues) {
  // 		return success;
  // 	} else {
  // 		return failure;
  // 	};
  ...
  // let testArr = [3, 34, 4, 12, 5, 2];
  // let actual = findPairForSum(testArr, 9);
  // let expected = [4, 5];
  // console.log(assertArraysEqual(actual, expected, "it returns the integer pair that equals the target sum"));
